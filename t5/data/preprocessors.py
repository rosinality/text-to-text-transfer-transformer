...
def pad_nonspaced_languages(x, text_key="text"):
    ...
    text = tf.strings.regex_replace(text, "([{}])".format(pattern), r" \1 ")
...
def trivia_qa(dataset):
    ...
    def triviaqa_question_answer_context(x):
        ...
        find_answers = tf.TensorArray(tf.bool, size=combination_size, dynamic_size=True)
        selected_answers = tf.TensorArray(
            tf.string, size=combination_size, dynamic_size=True
        )
        join_q_c = tf.TensorArray(tf.string, size=combination_size, dynamic_size=True)
        ...
        def body_fn(i, find_answers, selected_answers, join_q_c):
            ...
            find_a = tf.strings.regex_full_match(
                tf.strings.lower(c), tf.strings.lower(a_)
            )
        ...
        _, find_answers, selected_answers, join_q_c = tf.while_loop(
            cond_fn,
            body_fn,
            loop_vars=[tf.constant(0), find_answers, selected_answers, join_q_c],
        )
    ...
    def my_fn(x):
        ...
        return {
            "inputs": _string_join_simple(
                ["[NLU] Question: ", x["question"], " Answer: <extra_id_0>"]
            ),
            "targets": x["answer"]["value"],
            "answers": x["answer"]["aliases"],
        }
...
def ul2_trivia_qa_nocontext_oneshot(dataset):
    """Convert a TriviaQA example to multiple flattened examples.
    ...
    def my_fn(x):
        ...
        return {
            "inputs": _string_join_simple(
                [
                    "[NLU] Question: Which American-born Sinclair won the Nobel Prize for Literature in 1930?\nAnswer: sinclair lewis.\n\nQuestion: ",
                    x["question"],
                    "\nAnswer: <extra_id_0> .",
                ]
            ),
...
def sft_mmlu(dataset):
    def my_fn(x):
        """Create TriviaQA example."""
        zero_shot_question = tf.strings.split(
            tf.strings.split(x["prompt"], sep="\n\n")[-1], sep="\nAnswer"
        )[0]
        question = tf.strings.split(zero_shot_question, sep="\n")[0]

        answers = tf.strings.substr(
            zero_shot_question,
            tf.strings.length(question) + 1,
...
def lambada(dataset):
    ...
    def my_fn(example):
        ...
        except_last_word = tf.strings.substr(
            example, 0, tf.strings.length(example) - tf.strings.length(answer) - 1
        )
        return {
...
def ul2_lambada(dataset):
    ...
    def my_fn(example):
        ...
        return {
            "inputs": _string_join_simple(
                ["[NLU] ", except_last_word, " <extra_id_0>"]
            ),
            "targets": answer,
        }
...
def ul2_humaneval(dataset):
    ...
    def my_fn(example):
        ...
        return {
            "inputs": _string_join_simple(
                ["[S2S] ", prompt, example["prompt"], " <extra_id_0>"]
            ),
            "targets": example["canonical_solution"],
            "task_id": example["task_id"],
        }
...
def orca_sft(dataset):
    ...
    def my_fn(x):
        ...
        return {
            "inputs": tf.strings.join(
                [x["system_prompt"], "USER: ", x["question"], "ASSISTANT: "],
                separator="",
            ),
            "targets": x["response"],
        }
...
def random_split_text(
    ...
    def random_chunk(x, chunk_size, seed):
        ...
        chunk_num = tf.random.stateless_uniform(
            [], seed=seed, minval=0, maxval=num_chunks, dtype=tf.int32
        )
    ...
    def my_fn(x, seeds):
        ...
        num_segments = tf.cast(
            tf.math.ceil(tf.cast(n_words, tf.float32) / tf.cast(length, tf.float32)),
            tf.int32,
        )
...
def fill_in_the_blank(dataset, text_key="text", label="fill: "):
    ...
    def my_fn(x, seeds):
        ...
        p_break = tf.exp(
            tf.random.stateless_uniform(
                [], minval=min_log_p_break, maxval=max_log_p_break, seed=seeds[0]
            )
        )
        ...
        def one_random_break():
            pos = tf.random.stateless_uniform(
                [], minval=0, maxval=n_words - 1, dtype=tf.int32, seed=seeds[2]
            )
            return tf.one_hot(
                pos, n_words - 1, dtype=tf.bool, on_value=True, off_value=False
            )

        breaks = tf.cond(tf.math.reduce_any(breaks), lambda: breaks, one_random_break)
        ...
        for seq_id in [0, 1]:
            ...
            separator_strings = tf.where(in_my_seq, " ", tf.where(breaks, " X", ""))
            word_strings = tf.where(in_my_seq, words, "")
            all_strings = tf.stack([separator_strings, word_strings], axis=1)
            results.append(
                tf.strings.substr(tf.strings.reduce_join(all_strings), 1, tf.int32.max)
            )
        inputs = tf.stack(
            [tf.strings.join([label, results[0]]), tf.strings.join([label, results[1]])]
        )
...
def fill_in_the_blank_sized(
    ...
    def my_fn(x, seeds):
        ...
        input_ = tf.strings.strip(tf.strings.join([pre_blank, blank, post_blank], " "))
        input_ = tf.strings.join([label, input_])
        target = tf.strings.reduce_join(
            words[blank_start : blank_start + blank_size], separator=" "
        )
        return {"inputs": tf.strings.strip(input_), "targets": tf.strings.strip(target)}
...
def wsc(x):
    ...
    def _mark_span(text, span_str, span_idx, mark):
        ...
        pattern = tf.strings.regex_replace(pattern_tmpl, "N", tf.as_string(span_idx))
...
def record(dataset):
    ...
    def my_fn(x):
        ...
        passage = x["passage"]
        passage = tf.strings.regex_replace(
            passage, r"(\.|\?|\!|\"|\')\n@highlight\n", r"\1 "
        )
        ...
        strs_to_join = [
            ...
            x["query"],
            "entities:",
            tf.strings.reduce_join(x["entities"], separator=", "),
            "passage:",
            passage,
        ]
        joined = tf.strings.join(strs_to_join, separator=" ")
...
def next_sentence_prediction(
    ...
    def my_fn(x, seed):
        ...
        use_neighbors = tf.random.stateless_uniform(shape=[], seed=seed) < p_neighbors
        ...
        with tf.control_dependencies(
            ...
            return tf.strings.join(
                [
                    ...
                    "X",
                    tf.strings.reduce_join(words[pronoun_index + 1 :], separator=" "),
                ],
                separator=" ",
            )
...
def wnli_simple(x, label="wsc:"):
    ...
    def split_clean(s):
        ...
        words = [re.sub(r"(\.|,|\?|\!)$", "", w) for w in s.strip().lower().split(" ")]
    ...
    def get_pronoun_match(premise, hypothesis, index):
        ...
        if score:
            ...
            m = re.search(
                " ".join(pre_match + [r"(.+)"] + post_match), " ".join(hypothesis)
            )
            if not m:
        ...
        return PronounMatch(score=score, index_in_premise=index, candidate=candidate)
    ...
    def get_best_pronoun_match(premise, hypothesis):
        ...
        scoredpronouns = [
            get_pronoun_match(premise, hypothesis, index) for index in pronoun_indices
        ]
    ...
    def compute_inputs_and_targets(premise, hypothesis):
        ...
        match = get_best_pronoun_match(split_clean(premise), split_clean(hypothesis))
...
def rank_classification(
    ...
    if mode not in ("train", "eval", "fewshot_eval"):
        raise ValueError(
            "Mode must be one of 'train', 'eval', or 'fewshot_eval'. " f"Got '{mode}'."
        )
...
def rank_classification_formatter(
    ...
    Returns:
      A tf.data.Dataset containing 'idx', inputs', 'targets', and 'is_correct'.
    """
    if isinstance(inputs_formats, (list, tuple)) and isinstance(
        targets_formats, (list, tuple)
    ):
...
def format_options_arc(example):
    ...
    options = tf.reshape(
        tf.stack([char_options[: len(example["options"])], example["options"]], axis=1),
        [-1],
    )

    example["options"] = tf.strings.reduce_join(
        [options_prefix, tf.strings.reduce_join(options, separator=separator)],
    )
    return example
...
def prefix_lm(dataset, sequence_length, output_features, noise_density=0.5, max_prefix_length=None):
    ...
    ds = denoise(
        ...
        noise_density=noise_density,
        noise_mask_fn=functools.partial(random_prefix_noise_mask, max_prefix_length=max_prefix_length),
    )
    return ds
...
def single_example_select_random_chunk(
    ...
    if min_length is not None:
        length = tf.random.stateless_uniform(
            [], minval=min_length, maxval=max_length, dtype=tf.int32, seed=seeds[0]
        )
    else:
        length = max_length
...
def ul2_objective(
    ...
    use_causal_lm_task: bool = True,
    rates: Sequence[float] = (0.1, 0.1, 0.2, 0.6),
    mean_noise_span_lengths: Sequence[float] = (
        3.0,
        32.0,
    ),
    noise_densities: Sequence[float] = (
        0.15,
        0.5,
    ),
    shard_ds: bool = True,
    optional_task_prefixes: Optional[Sequence[str]] = [
        "[NLU] ",
        "[NLU] ",
        "[S2S] ",
        "",
    ...
    if optional_task_prefixes:  # Ensure each task has a prefix.
        num_tasks = (
            len(noise_densities) + int(use_prefix_lm_task) + int(use_causal_lm_task)
        )
    ...
    if merge_examples_to_reduce_padding:
        ds = reduce_concat_tokens(ds, feature_key="targets", batch_size=128)
        num_shards = (
            len(input_lengths) + int(use_prefix_lm_task) + int(use_causal_lm_task)
        )
    if shard_ds:
    ...
    for input_length, (noise_span_length, noise_density), i in hyperparams:
        ...
        ds = denoise(
            ...
            noise_density=noise_density,
            noise_mask_fn=functools.partial(
                random_spans_noise_mask, mean_noise_span_length=noise_span_length
            ),
            input_feature_key=input_feature_key,
        )
    ...
    if use_prefix_lm_task:
        offset = -1 if not use_causal_lm_task else -2
        ...
        if optional_task_prefixes:
            ds = prepend_prompt(
                ds,
                output_features,
                prompt_mode=optional_task_prefixes[offset],
                mode=optional_task_prefixes[offset],
            )
        processed_ds.append(ds)

    if use_causal_lm_task:
        ds = ds_shards[-1]
...
def reduce_concat_tokens(
    dataset, feature_key="targets", batch_size=128, **unused_kwargs
):
        ...
        def truncate_inputs():
            ...
            def answer_in_context(context, answer):
                ...
                strided = tf.nn.conv1d(
                    conv_inp, tf.reshape(filters, [ans_len, 1, ans_len]), 1, "VALID"
                )
            ...
            def slice_inputs(inputs, answer_len, pos_mask, seed=None):
                ...
                start_range_max = (
                    tf.minimum(ans_start_pos, inputs_len - max_input_tokens) + 1
                )
...
@gin.configurable()
def unsupervised(
    dataset, preprocessors=None, output_features=None, sequence_length=None
):
...
def split_tokens(
    ...
    **unused_kwargs,
    ...
    def _split_tokens(x, seed):
        ...
        if passthrough_feature_keys:
            passthrough = {k: v for k, v in x.items() if k in passthrough_feature_keys}
    ...
    def _strip_padding_and_merge_passthrough(inputs, orig_lengths, passthrough=None):
        output = {}
...
@gin.configurable
def split_tokens_to_targets_length(dataset, sequence_length, output_features, **kwargs):
...
@gin.configurable
def split_tokens_to_random_length(dataset, sequence_length, output_features, **kwargs):
    max_tokens = sequence_length["inputs"]
    if output_features["inputs"].add_eos:
        # Leave room to insert an EOS token.
        max_tokens -= 1
    ...
    return split_tokens(
        dataset, min_tokens_per_segment=8, max_tokens_per_segment=max_tokens, **kwargs
    )


@gin.configurable
def concatenate_and_split_to_fixed_length(
    dataset, sequence_length, output_features, feature_key="targets", **unused_kwargs
):
...
def filter_by_string_length(
    dataset, feature_key="targets", min_length=1, max_length=1000000, **unused_kwargs
):
    ...
    def my_fn(x):
        ...
        return tf.logical_and(
            tf.greater_equal(l, min_length), tf.less_equal(l, max_length)
        )
...
class DenoiseNoiseMaskFn(Protocol):
    ...
    def __call__(
        self, num_tokens: tf.Tensor, noise_density: float, seeds: tf.Tensor
    ) -> tf.Tensor:
...
class DenoiseInputsFn(Protocol):
    ...
    def __call__(
        self, tokens: tf.Tensor, noise_mask: tf.Tensor, vocabulary, seeds: tf.Tensor
    ) -> tf.Tensor:
...
def single_example_denoise(
    ...
    *,
    ...
    """
    if passthrough_feature_keys and (
        input_feature_key in passthrough_feature_keys
    ...
    if (
        input_feature_key in output_features
        and vocabulary != output_features[input_feature_key].vocabulary
    ):
        raise ValueError(
...
@gin.configurable()
def regular_noise_mask(
    length, noise_density, seeds, min_span_length=1, max_span_length=5
):
    ...
    offset = tf.random.stateless_uniform(
        [], maxval=period, dtype=tf.int32, seed=seeds[1]
    )
...
def random_spans_noise_mask(
    length, noise_density, seeds, mean_noise_span_length=3.0, random_roll=False
):
    ...
    if random_roll:
        ...
        offset = tf.random.stateless_uniform(
            [1], seed=roll_seed, dtype=tf.int32, minval=0, maxval=length
        )[0]
...
@gin.configurable()
def random_prefix_noise_mask(length, noise_density, seeds, max_prefix_length=None):
    ...
    min_input_tokens = tf.minimum(
        ...
        tf.maximum(
            ...
            tf.cast(
                tf.math.round(
                    (1 - 2 * noise_density) * tf.cast(max_input_tokens, tf.float32)
                ),
                tf.int32,
            ),
        ),
...
def noise_token_to_sentinel(tokens, noise_mask, vocabulary, seeds):
    ...
    """
    del seeds
    return tf.where(noise_mask, tf.cast(sentinel_id(vocabulary), tokens.dtype), tokens)
...
def noise_span_to_sentinel(tokens, noise_mask, vocabulary, seeds):
    ...
    del seeds
    tokens = tf.where(
        noise_mask, tf.cast(sentinel_id(vocabulary), tokens.dtype), tokens
    )
...
def nonnoise_span_to_sentinel(tokens, noise_mask, vocabulary, seeds):
    return noise_span_to_sentinel(tokens, tf.logical_not(noise_mask), vocabulary, seeds)
...
def noise_token_to_gathered_token(tokens, noise_mask, vocabulary, seeds):
    ...
    indices = tf.random.stateless_uniform(
        shape=tf.shape(tokens), maxval=tf.size(tokens), dtype=tf.int32, seed=seeds[0]
    )
...
def noise_token_to_random_token(
    tokens, noise_mask, vocabulary, seeds, num_reserved_tokens=3
):
...
def noise_token_to_random_token_or_sentinel(
    tokens, noise_mask, vocabulary, seeds, random_prob=0.1
):
    ...
    """
    use_random = (
        tf.random.stateless_uniform(tf.shape(tokens), seed=seeds[0]) < random_prob
    )
    return tf.where(
        use_random,
        noise_token_to_random_token(tokens, noise_mask, vocabulary, seeds=seeds[1:]),
...
def trim_and_pad_dataset(dataset, sequence_length):
    ...
    return seqio.utils.trim_and_pad_dataset(dataset, feature_lengths=sequence_length)
...
def pack_prefix_lm_encoder_decoder(ds, sequence_length, pad_id=0):
    ...
    def pack_examples(example_pair, seed):
        split_point = tf.random.stateless_uniform(
            (), minval=1, maxval=packed_length, seed=seed, dtype=tf.int32
        )
        inputs = tf.concat(
            [
        ...
        encoder_segment_ids = (
            tf.cast(tf.range(packed_length) >= split_point, tf.int32) + 1
        )
        decoder_segment_ids = (
            tf.cast(tf.range(packed_length) >= (packed_length - split_point), tf.int32)
            + 1
        )
        ...
        encoder_positions = tf.concat(
            [tf.range(split_point), tf.range(packed_length - split_point)], axis=0
        )
        encoder_positions = tf.reshape(encoder_positions, (packed_length,))
        decoder_positions = tf.concat(
            [tf.range(packed_length - split_point), tf.range(split_point)], axis=0
        )
        decoder_positions = tf.reshape(decoder_positions, (packed_length,))
        decoder_loss_weights = tf.cast(tf.not_equal(targets, pad_id), dtype=tf.int32)
        return {
...
def pack_prefix_lm_decoder_only(
    ds, sequence_length, loss_on_targets_only=True, pad_id=0
):
